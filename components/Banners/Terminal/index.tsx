...
export default function TerminalBanner({
  ...
  const recentActivity = useMemo(() => {
    const all = [
      ...(posts
        ?.slice(0, 3)
        .map((p) => ({ label: 'POST', title: p.title, date: p.dateString })) ||
        []),
      ...(projects
        ?.slice(0, 3)
        .map((p) => ({ label: 'PROJ', title: p.name, date: p.dateString })) ||
        []),
    ].sort(
      (a, b) =>
  ...
  return (
    <Card
      className={`relative overflow-hidden font-mono text-xs md:text-sm bg-[#fbf1c7] dark:bg-[#1d2021] border-[#d5c4a1] dark:border-[#3c3836] ${className}`}
      isPreview
    >
      {/* Terminal Header */}
      ...
      <div className='p-4 space-y-5 min-h-[250px]'>
        ...
        <div className='flex flex-col md:flex-row gap-6'>
          <div className='flex-1 space-y-1'>
            <div className='text-[#98971a] dark:text-[#b8bb26] font-bold'>
              OS:{' '}
              <span className='text-[#3c3836] dark:text-[#ebdbb2] font-normal'>
                {systemInfo.os}
              </span>
            </div>
            <div className='text-[#98971a] dark:text-[#b8bb26] font-bold'>
          ...
          <div className='flex-1 space-y-1'>
            <div className='text-[#d79921] dark:text-[#fabd2f] font-bold uppercase tracking-tighter mb-1'>
              Database Sync
            </div>
            <div className='text-[#689d6a] dark:text-[#8ec07c]'>
              {progressBar()}
            </div>
            <div className='text-[10px] text-[#928374] animate-pulse'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-4 gap-3'>
          {stats.map((stat) => (
            <div
              key={stat.label}
              className='bg-[#ebdbb2] dark:bg-[#282828] border border-[#d5c4a1] dark:border-[#3c3836] p-2 rounded relative overflow-hidden group'
            >
              <div className='absolute top-0 right-0 p-1 opacity-10 group-hover:opacity-20 transition-opacity'>
                <Icon
                  name='database'
                  className='size-8 text-[#3c3836] dark:text-[#ebdbb2]'
                />
              </div>
              <div className='text-[9px] text-[#928374] font-bold mb-1'>
                {stat.label}
              </div>
        ...
        <div className='space-y-2'>
          ...
          <div className='space-y-1 font-mono text-[11px] md:text-xs'>
            {recentActivity.map((act, i) => (
              <div
                key={i}
                className='flex items-center gap-3 py-0.5 hover:bg-[#ebdbb2] dark:hover:bg-[#32302f] transition-colors group cursor-default'
              >
                <span className='text-[#928374] w-12 shrink-0'>
                  [{act.label}]
                </span>
                <span className='text-[#3c3836] dark:text-[#ebdbb2] truncate flex-1'>
                  {act.title}
                </span>
                <span className='text-[#458588] dark:text-[#83a598] shrink-0'>
                  {act.date}
                </span>
                <span className='text-[#98971a] dark:text-[#b8bb26] shrink-0 opacity-0 group-hover:opacity-100'>
                  READY
                </span>
              </div>
            ))}
        ...
        <div className='flex items-center gap-2 pt-1 border-t border-[#d5c4a1] dark:border-[#3c3836]/30'>
          <span className='text-[#98971a] dark:text-[#b8bb26] font-bold'>
            zulfikar@homepage
          </span>
          <span className='text-[#3c3836] dark:text-[#ebdbb2]'>:</span>
          <span className='text-[#458588] dark:text-[#83a598]'>~</span>
          <span className='text-[#3c3836] dark:text-[#ebdbb2] font-bold'>
            $
          </span>
          <div className='flex items-center'>
            <span className='text-[#3c3836] dark:text-[#ebdbb2]'>
              watch -n 1 stats.sh
            </span>

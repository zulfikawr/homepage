...
export default function Aside({ preNext }: { preNext: any }) {
  ...
  useEffect(() => {
    ...
    const scrollHandler = () => {
      ...
      const prevHeader = document.getElementById(
      ...
      {headersEl.length ? (
        <div>
          <div className='rounded-xl border bg-white shadow-sm dark:border-gray-800 dark:bg-gray-800'>
            ...
            <ul
              className='mask-y max-h-[60vh] overflow-hidden overflow-y-auto overscroll-contain px-3 py-3 text-xl text-gray-500 transition-colors duration-300 group-hover:text-gray-700 dark:text-gray-400 dark:group-hover:text-gray-300'
              id='toc'
            >
              {headersResult &&
